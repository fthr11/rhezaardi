...
@section('content')
    <div class="max-w-4xl mx-auto px-6 pt-14 sm:pt-16">
        ...
        {{-- ====== PAGE TITLE ====== --}}
        <h1
            class="font-serif font-semibold text-[2.4rem] sm:text-[4.5rem] font-black tracking-tight leading-[1.05] text-neutral-900 mb-8">
            {{ $blog->title }}
        </h1>

        {{-- ====== HERO IMAGE ====== --}}
        <div class="w-full mb-10 overflow-hidden rounded-sm">
            <img src="{{ asset('images/blogs/' . $blog->thumbnail) }}" alt="{{ $blog->title }}" width="800" height="450"
                fetchpriority="high" decoding="async" class="w-full aspect-[16/9] object-cover">
        </div>

        {{-- ====== BODY CONTENT ====== --}}
        <x-content :content="$blog->content" :images="$blog->images" :title="$blog->title" disk="blogs" />

        <x-embed :data="$blog" :url="$blog->embed" />

        {{-- ====== DIVIDER ====== --}}
        <hr class="border-neutral-200 mb-10">

        {{-- ====== SHARE / TAGS ROW ====== --}}
        <div class="flex flex-wrap items-center justify-between gap-4 mb-20">
