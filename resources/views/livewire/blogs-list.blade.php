<div x-data="{ openSort: false }">
    ...
    <!-- Filter Bar: Sorting and Active Tags -->
    <div class="flex flex-wrap items-center gap-4 mb-10">
        ...
        <div class="relative">
            ...
            <div x-show="openSort" x-transition:enter="transition ease-out duration-100"
                ...
                class="absolute left-0 mt-2 w-48 bg-white border border-neutral-200 rounded-2xl shadow-xl z-20 py-2 overflow-hidden max-h-72 overflow-y-auto">
                {{-- Basic Sorts --}}
                ...
                @if (count($availableYears))
                    <div class="h-px bg-neutral-100 my-1 mx-2"></div>
                    @foreach ($availableYears as $year)
                        <button wire:click="setSort('{{ $year }}')" @click="openSort = false"
                            class="w-full text-left px-4 py-2 text-[13px] hover:bg-neutral-50 {{ $sort == $year ? 'font-bold' : '' }}">
                            {{ $year }}
                        </button>
                    @endforeach
                @endif
        ...
        {{-- ====== Active Tag Badge (Beside Sort) ====== --}}
        @if ($tag)
            <div
                class="flex items-center gap-2 bg-neutral-900 text-white px-4 py-2.5 rounded-full text-[13px] font-medium shadow-sm animate-in fade-in slide-in-from-left-2 duration-300">
                <span class="opacity-70 text-[10px] uppercase tracking-widest font-bold mr-0.5">{{ __('Tag') }}:</span>
                <span class="font-semibold">#{{ $tag }}</span>
                <button wire:click="clearTag" class="ml-1 opacity-60 hover:opacity-100 transition-opacity"
    ...
    <!-- Active Filter Title (Mobile or Small screens context) -->
    @if ($tag)
        <div class="mb-8 block animate-in fade-in duration-500">
            <h2 class="text-[15px] text-neutral-500 font-medium">
                {{ __('results') }}: <span class="text-neutral-900 border-b border-neutral-300">#{{ $tag }}</span>
            </h2>
        </div>
    @endif

    <!-- Skeleton Loading Grid -->
    <div wire:loading.grid wire:target="setSort, setTag, clearTag, gotoPage"
        class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-3 gap-y-12">
        @foreach (range(1, max(3, count($blogs))) as $i)
            <x-skeleton-card />
        @endforeach
    </div>

    <!-- Blog Grid -->
    <div wire:loading.remove wire:target="setSort, setTag, clearTag, gotoPage">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-3 gap-y-12">
            @forelse ($blogs as $post)
                <x-card :item="$post" type="blog" :imgLoading="$loop->iteration <= 3 ? 'eager' : 'lazy'"
            ...
            @empty
                <div class="col-span-full py-24 text-center">
                    ...
                    <p class="text-[14px] text-neutral-400">
                        @if($tag)
                            {{ __('No posts found with tag') }} #{{ $tag }}.
                        @else
                            {{ __('No posts found.') }}
                        @endif
                    </p>
